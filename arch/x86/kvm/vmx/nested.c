...
u64 g2va_to_g1pa(struct kvm_vcpu *vcpu, struct vmcs12* vmcs12, u64 addr){
	...
	// real_gfn = kvm_translate_gpa(vcpu, vcpu->arch.walk_mmu, gfn_to_gpa(gfn), 0, NULL);
	real_gfn = vcpu->arch.walk_mmu->gva_to_gpa(vcpu, vcpu->arch.walk_mmu, gfn_to_gpa(gfn), 0, NULL);
	// FIXME TODO this is just a guess and probably not correct
